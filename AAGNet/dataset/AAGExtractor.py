...
def process_one_file(args):
    ...
    try:
        ...
        return [str(fn.stem), None, str(e)]

    return [str(fn.stem), out, None]
...
def main(args):
    ...
    results = [[fn, out] for fn, out, err in raw_results if err is None]
    failures = [(fn, err) for fn, out, err in raw_results if err is not None]

    if failures:
        with open(osp.join(output_path, 'failed_extractions.txt'), 'w', encoding='utf8') as f:
    ...
    gc.collect()
    print(f"Processed {len(results)} files, {len(failures)} failed (see failed_extractions.txt).")
